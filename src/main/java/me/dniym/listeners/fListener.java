...
public class fListener implements Listener {
    ...
    private final HashMap<Player, Long> swapDelay = new HashMap<Player, Long>();
        ...
        if (Protections.DisableRidingExploitableMobs.isEnabled()) {
            if (IllegalStack.hasChestedAnimals()) {
                if ((e.getMount() instanceof Mule || e.getMount() instanceof Donkey || e.getMount() instanceof ChestedHorse) ||
                    ...
                    if (!IllegalStackAction.isCompleted(Protections.DisableRidingExploitableMobs, e.getEntity(), null, null, e.getMount(), null))
                        return;
                ...
                if (e.getMount() instanceof Horse && IllegalStackAction.isCompleted(Protections.DisableRidingExploitableMobs, e.getEntity(), null, null, e.getMount(), null)) {
                    e.getMount().eject();
